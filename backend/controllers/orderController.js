...
const createOrder = async (req, res) => {
    try {
        ...
        if (!orderItems && orderItem.length === 0){
            res.status(400);
            throw new Error("There are not items to order!");
        }
        ...
        const dbOrderItems = orderItems.map((itemsFromClient) => {
            ...
            if (!matchingItemFromDB) {
                res.status(404)
                throw new Error(`Item not found: ${itemsFromClient._id}`);
            }
...
const  findOrderById = async (req, res) => {
   try {
        const order = await Order.findById(req.params.id).populate("user", "usename email");
        if (order){
            res.json(order);
        }else {
            res.status(404)
